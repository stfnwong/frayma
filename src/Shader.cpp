...
std::string load_shader(const std::string& filename)
{
    ...
    std::cout << "[" << __func__ << "] shader text:" << std::endl;
...
Shader::Shader(const std::string& filename)
{
    ...
    this->gl_shader[0] = create_shader(load_shader(filename + ".vert"), GL_VERTEX_SHADER);
    this->gl_shader[1] = create_shader(load_shader(filename + ".frag"), GL_FRAGMENT_SHADER);
